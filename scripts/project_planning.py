...
def convert_categoricals(df: pd.DataFrame, extra_categoricals=None, max_unique_threshold=50) -> pd.DataFrame:
    # Step 1: Auto-detect object/string columns with limited unique values
    auto_categoricals = [
        col for col in df.select_dtypes(include=["object", "string"]).columns
        if df[col].nunique(dropna=False) <= max_unique_threshold
    ]
    ...
    # Step 3: Combine, deduplicate, and exclude 'TransactionMonth'
